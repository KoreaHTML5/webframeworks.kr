...
watch는 특정 데이터를 바라보고 있다가 변경되면 실행됩니다.  
...
newVal todoList가 변한 값이고, oldVal는 todoList가 변하기 전 값입니다.  
...
그 이유는 코드 반복이 더 적고 우수한 성능을 나타낸다고 합니다.    

그럼 watch는 언제 쓰면 좋을까요?   
특정 데이터(하나의 데이터)를 바라보다가 변경된 데이터로 어떤 로직을 수행해야 할 때가 적절합니다.  
computed 같은 경우는 종속된 데이터를 받아서 변형한 데이터를 리턴하지만 watch 같은 경우는 리턴할 필요없이 함수 내에서 필요한 메소드나 기존 데이터에 새로운 데이터를 정의할 수 있습니다.  
