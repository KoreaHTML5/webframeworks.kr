...
하지만 아직 템플릿과 virtual Dom(가상돔)은 아직 마운트되거나 렌더링 되지 않았습니다.  

### beforeMount
이 부분은 마운트 직전에 호출되는 부분인데 대부분의 사용자들이 이 시점에서 어떠한 작업도 안한다고 합니다.  
가장 뚜렷한 이유는 서버사이드 렌더링에서는 호출되지 않는 점이네요.  
...
총 4가지 beforeCreate, created, beforeMount, mounted가 상기 설명드린 시점에 실행되었습니다.  
...
그럴 때 이 훅을 참조하면서 사용하시면 훨씬 더 잘 이해가 될 겁니다.    
