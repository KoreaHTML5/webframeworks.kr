...
위와 같이 프로젝트 이름과, 설명, 개발자를 간단히 입력해주면 템플릿이 생성됩니다.  
to get started의 설명과 같이 해당 프로젝트 경로로 들어가 npm intall과 npm run dev를 실행해봅시다.  
...
#### package.json
script 실행(npm run dev 등) 및 설정한 모듈과 관련한 정보를 확인할 수 있습니다.  
